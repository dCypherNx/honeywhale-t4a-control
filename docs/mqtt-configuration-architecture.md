...
`AndroidLocationProvider` is the only Android location adapter. It converts framework `Location` objects into provider-neutral `LocationSnapshot` values. It is inactive while BLE is disconnected. While connected and stationary it uses a 20-second balanced-power cadence with a 10 m minimum-distance threshold; when DP 2 reports more than 1 km/h it switches to high accuracy, a 3-second cadence and a 2 m minimum-distance threshold when fine location is available.
...
- BLE connected and DP 2 <= 1 km/h: balanced-power updates targeted at every 20 seconds, with a 10 m minimum-distance threshold.
- BLE connected and DP 2 > 1 km/h: high-accuracy updates targeted at every 3 seconds when fine location is available, with a 2 m minimum-distance threshold.
