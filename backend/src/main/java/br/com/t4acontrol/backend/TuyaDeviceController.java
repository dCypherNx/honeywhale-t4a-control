...
/**
 * Simple Tuya-based DeviceController implementation that delegates to ThingHomeSdk.
 * This class centralizes SDK interactions so the UI can stay thin.
 */
public class TuyaDeviceController implements DeviceController {
    ...
    public void setDeviceEventsListener(DeviceEventsListener l) { this.listener = l; }

    @Override
    public User getUser() { return ThingHomeSdk.getUserInstance().getUser(); }
    ...
    @Override
    public void queryHomeList(IThingGetHomeListCallback callback) { ThingHomeSdk.getHomeManagerInstance().queryHomeList(callback); }

    @Override
    public void getHomeDetail(long homeId, IThingHomeResultCallback callback) { ThingHomeSdk.newHomeInstance(homeId).getHomeDetail(callback); }

    @Override
    public void getActivatorToken(long homeId, IThingActivatorGetToken callback) { ThingHomeSdk.getActivatorInstance().getActivatorToken(homeId, callback); }

    @Override
    public void startLeScan(LeScanSetting setting, ScanCallback callback) {
        ThingHomeSdk.getBleOperator().startLeScan(setting, scanDevice -> {
    ...
    @Override
    public com.thingclips.smart.sdk.bean.DeviceBean getDeviceBean(String devId) { return ThingHomeSdk.getDataInstance().getDeviceBean(devId); }
    ...
    @Override
    public void publishDps(IThingDevice device, String json, IResultCallback callback) { device.publishDps(json, callback); }

    @Override
    public void readBluetoothRssi(String mac, RssiCallback callback) {
        ThingHomeSdk.getBleOperator().readBluetoothRssi(mac, (isSuccess, rssi) -> { if (callback != null) callback.onResult(isSuccess, rssi); });
    }

    @Override
    public void connectBleDevice(List<com.thingclips.smart.android.ble.builder.BleConnectBuilder> builders) {
        ThingHomeSdk.getBleManager().connectBleDevice(builders);
    }

    @Override
    public void removeDevice(IThingDevice device, IResultCallback callback) { device.removeDevice(callback); }
}
