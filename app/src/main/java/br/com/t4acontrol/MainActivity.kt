...
class MainActivity : ComponentActivity(), T4ASession.Listener {
    ...
    private val uiActions = object : T4AUiActions {
        ...
        override fun pair() = session.pair()
        ...
        override fun openMqttSettings() {
            startActivity(Intent(this@MainActivity, MqttSettingsActivity::class.java))
        }
        ...
        override fun confirmUnpair() = this@MainActivity.confirmUnpair()

        override fun setRawLogFilter(filter: String) {
            rawLogFilter = filter
        }

        override fun sectionExpanded(key: String, defaultExpanded: Boolean): Boolean =
            preferences().getBoolean("section_$key", defaultExpanded)

        override fun setSectionExpanded(key: String, expanded: Boolean) {
            preferences().edit().putBoolean("section_$key", expanded).apply()
        }

        override fun copyRawLog() = this@MainActivity.copyRawLog()

        override fun saveRawLog() = this@MainActivity.saveRawLog()
    }
    ...
    private fun showBatteryRechargePromptIfNeeded(current: T4AState?) {
        ...
        batteryRechargeDialog = android.app.AlertDialog.Builder(this)
            ...
            .setPositiveButton(getString(R.string.battery_recharge_confirm)) { _, _ ->
                session.resolveBatteryRecharge(true)
                batteryRechargeDialog = null
            }
            .setNegativeButton(getString(R.string.battery_recharge_reject)) { _, _ ->
